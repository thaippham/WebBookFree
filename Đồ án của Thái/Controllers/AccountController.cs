...
namespace Đồ_án_của_Thái.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, string imgbase64)
        {
            var categorys = _dbContext.Categories.ToList();
            ViewBag.Categories = categorys;
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    string emailConfirmationHtml = System.IO.File.ReadAllText(Server.MapPath("~/Content/htmlConfirmEmail/EmailConfirm.html"))
                        .Replace("@Model.Name", user.Name)
                        .Replace("@Model.CallbackUrl", callbackUrl);
                    var message = new IdentityMessage
                    {
                        Destination = user.Email,
                        Subject = "Confirm Email to Login",
                        Body = emailConfirmationHtml
                    };
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterAdmin(RegisterAdminViewModels model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                    var result1 = UserManager.AddToRole(user.Id, model.Name);
                }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                string resetPasshtml = System.IO.File.ReadAllText(Server.MapPath("~/Content/htmlConfirmEmail/ResetPass.html"))
                        .Replace("@Model.Name", user.Name)
                        .Replace("@Model.CallbackUrl", callbackUrl);
                var message = new IdentityMessage
                {
